...
function formatPost(post, req, sendingUserId) {
    ...
    if(post.answers.length > 0)
        layout.answers.text = post.answers[0].text;
    else
        layout.answers.text = '';

    layout.answers.count = post.answers.length;
    post.answers.forEach(lk => {
        if (lk.user_id.equals(sendingUserId))
        ...
        .exec((err, post) => {
            ...
            if (post) {
                ...
                post.answers.forEach(a => {
                    answers.push(a.user_id.toString());
                });
...
//function to calculate the ammount of time passed since the post
...
exports.loadPosts = (objectIdArrayOfUser, sendingUserId, req, cb) => {
    ...
    Post.find({ user_id: objectIdArrayOfUser })
        .sort({ date: -1 })
        .limit(180) //limitting the posts to 180 to stop overtime
    ...
    Post.findById(post_id)
        ...
        .exec((err, post) => {
            ...
            post.answers.forEach(ans => {
                let dat = {};
                dat.user_id = ans.user_id._id;
                /* if (lk._id.equals(sendingUserId))
                    dat.isLiked = 1; */
                dat.full_name = ans.user_id.full_name;
                dat.user_name = ans.user_id.user_name;
                dat.text = ans.text;
                dat.thumb_image = `${req.app.locals.dat.domain}/thumb/${ans.user_id.thumb_image}`;
                console.log(dat);
