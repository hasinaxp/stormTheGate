...
router.post('/answer', (req, res) => {
    ...
    if (errors) {
        ...
        FUNC.postExists(req.body.post_id, res, () => {
            //if the user exists
            UD.existsUser(req.body.user_id, 'user_id', res, () => {
                //checking if the user has already answered the post
                FUNC.postAnswerExists(req.body.post_id, req.body.user_id, (condition) => {
                    ...
                    if (!condition) {
                        ...
                        Post.findByIdAndUpdate(req.body.post_id, { $addToSet: { answers:  answer} })
                            .exec((err, p) => {
                                if (err) console.log(err);
                                if (p) {
                                    res.json({
                        ...
                        Post.findByIdAndUpdate(req.body.post_id, { $pull: { answers: { user_id : mongoose.Types.ObjectId(req.body.user_id)} } })
                            .exec((err, p) => {
                                if (err) console.log(err);
                                if (p) {
                                    res.json({
...
router.post('/answer/load', (req, res) => {
    ...
    if (errors) {
        ...
        UD.existsUser(req.body.user_id, 'user_id', res, () => {
            //if post exists
            FUNC.postExists(req.body.post_id, res, () => {
                FUNC.loadAnswers(req.body.post_id, req.body.user_id, req, (answerList) => {
                    res.json({
                        answers: answerList,
                        status: 1
                    });
